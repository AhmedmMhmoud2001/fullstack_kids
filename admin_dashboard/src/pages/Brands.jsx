...
const Brands = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response;
            if (editingBrand) {
                response = await brandsApi.update(editingBrand.id, formData);
            } else {
    ...
    return (
        <div className="space-y-6 overflow-x-hidden">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Our Brands</h1>
                    <p className="text-sm md:text-base text-gray-600 mt-1">Manage symbols of quality and trust</p>
                </div>
                <button
                ...
                {filteredBrands.map((brand) => (
                    <div key={brand.id} className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100 hover:shadow-xl transition-all group">
                        <div className="h-48 bg-gray-50 flex items-center justify-center p-8 relative">
                            {brand.image ? (
                                <img src={brand.image} alt={brand.name} className="max-h-full max-w-full object-contain group-hover:scale-105 transition-transform duration-500" />
                            ) : (
                                <div className="text-gray-300 font-black text-4xl select-none">{brand.name.substring(0, 2).toUpperCase()}</div>
                            )}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[1000] p-4 animate-fade-in">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-slide-in-bottom">
                        <div className="px-6 md:px-8 py-4 md:py-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50 sticky top-0 bg-white z-10">
                            <h2 className="text-xl md:text-2xl font-black text-slate-900 uppercase tracking-tight">
                                {editingBrand ? 'Modify Brand' : 'Register Brand'}
                            </h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-4 md:space-y-6">
                            ...
                            <div className="flex items-center gap-3 p-4 bg-blue-50/50 rounded-xl">
                                ...
                                />
                                <label htmlFor="isActive" className="text-sm text-slate-700 font-bold cursor-pointer">Set as Active Brand</label>
                            </div>
